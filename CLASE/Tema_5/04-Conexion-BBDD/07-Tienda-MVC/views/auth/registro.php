...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($valor_usuario) && !empty($valor_password)) {
        try {
            $register = $restaurante->createRestaurante(null, $valor_usuario, $valor_password, null, null, null, null);
            if ($register) $message = 'Restaurante registrado';
            else $err = 'Error al registrar el restaurante';
        } catch (Exception $e) {
            print "¡Error!: " . $e->getMessage() . "<br>";
        }
    } else {
        if (empty($valor_usuario)) $err = 'Correo vacío';
